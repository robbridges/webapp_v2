...
func (g Galleries) Images(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, models.ErrNoData) {
			http.Error(w, "Image not found", http.StatusFound)
		}
		logger.Create(err)
		http.Error(w, "Something went wrong", http.StatusInternalServerError)
		return
	...
	for _, fileHeader := range fileHeaders {
		...
		if err != nil {
			...
			if errors.As(err, &fileError) {
				msg := fmt.Sprintf("%v has an invalid content type or extension, only png, jpeg, and gifs are "+
					"accepted", fileHeader.Filename)
